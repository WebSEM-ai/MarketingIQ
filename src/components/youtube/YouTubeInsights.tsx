...
function renderMarkdown(text: string) {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
      elements.push(
        <h4 key={i} className="text-sm font-semibold text-red-400 mt-3 mb-1">
          {trimmed.replace(/\*\*/g, "")}
        </h4>
      );
      return;
    ...
    if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
      elements.push(
        <div key={i} className="flex items-start gap-2 mb-1 pl-2">
          <span className="text-red-500 mt-1.5 text-[6px]">&#9679;</span>
    ...
    if (numberedMatch) {
      elements.push(
        <div key={i} className="flex items-start gap-2 mb-1 pl-2">
          <span className="text-red-500 text-xs font-medium min-w-[16px]">
            {numberedMatch[1]}.
          </span>
...
export default function YouTubeInsights({ insights }: YouTubeInsightsProps) {
  return (
