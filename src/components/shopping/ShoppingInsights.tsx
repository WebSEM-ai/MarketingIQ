...
function renderMarkdown(text: string) {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
      elements.push(
        <h4 key={i} className="text-sm font-semibold text-orange-400 mt-3 mb-1">
          {trimmed.replace(/\*\*/g, "")}
        </h4>
      );
      return;
    ...
    if (numberedMatch) {
      elements.push(
        <div key={i} className="flex items-start gap-2 mb-1 pl-2">
          <span className="text-orange-500 text-xs font-medium min-w-[16px]">
            {numberedMatch[1]}.
          </span>
...
export default function ShoppingInsights({ insights }: ShoppingInsightsProps) {
  return (
