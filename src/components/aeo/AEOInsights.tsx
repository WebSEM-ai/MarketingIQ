...
function renderMarkdown(text: string) {
  ...
  lines.forEach((line, i) => {
    ...
    if (numberedMatch) {
      elements.push(
        <div key={i} className="flex items-start gap-2 mb-1 pl-2">
          <span className="text-cyan-500 text-xs font-medium min-w-[16px]">
            {numberedMatch[1]}.
          </span>
...
export default function AEOInsights({ insights }: AEOInsightsProps) {
  return (
    <div className="bg-gray-900/50 border border-gray-800/50 rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-xs font-semibold text-cyan-500 uppercase tracking-wider">
          Insights AEO Strategice
        </h3>
      </div>
